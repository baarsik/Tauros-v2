...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding, int rounding_corners)
{
    ...
    {
        window->DrawList->AddRect(p_min + ImVec2(1,1), p_max + ImVec2(1,1), GetColorU32(ImGuiCol_BorderShadow), rounding, rounding_corners, border_size);
...
bool ImGui::Combo(const char* label, int* current_item, std::vector<std::string> items, int popup_max_height_in_items)
{
	if (items.empty()) return false;
	auto value_changed = Combo(label, current_item, Items_VectorGetter, static_cast<void*>(&items), items.size(), popup_max_height_in_items);
	return value_changed;
}
