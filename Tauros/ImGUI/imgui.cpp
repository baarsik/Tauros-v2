...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int rounding_corners)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding, rounding_corners);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    bool hovered, held;
	const auto pressed = ButtonBehavior(total_bb, id, &hovered, &held);
    if (pressed)
        *v = !(*v);

	RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_CheckMark), true, style.FrameRounding);
	const auto diff = (check_bb.Max - check_bb.Min) * 0.05f;
	RenderFrame(check_bb.Min + diff, check_bb.Max - diff, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
	if (*v)
	{
		const auto check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const auto pad = ImMax(1.0f, static_cast<float>(static_cast<int>(check_sz / 6.0f)));
		...
		{
			ImVec2(check_mark_bb.GetCenter().x - check_mark_bb.GetWidth() / 2.05f, check_mark_bb.GetCenter().y),
			ImVec2(check_mark_bb.GetCenter().x - check_mark_bb.GetWidth() / 10.f, check_mark_bb.GetCenter().y + check_mark_bb.GetHeight() / 2.05f),
			ImVec2(check_mark_bb.GetTR())
		};
		window->DrawList->AddPolyline(points, 3, GetColorU32(ImGuiCol_CheckMark), false, 3.f, true);
	}
