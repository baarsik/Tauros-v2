...
void ImGui_ImplDX9_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    ImDrawIdx* idx_dst;
    if (g_pVB->Lock(0, static_cast<UINT>(draw_data->TotalVtxCount * sizeof(CUSTOMVERTEX)), reinterpret_cast<void**>(&vtx_dst), D3DLOCK_DISCARD) < 0)
        return;
    if (g_pIB->Lock(0, static_cast<UINT>(draw_data->TotalIdxCount * sizeof(ImDrawIdx)), reinterpret_cast<void**>(&idx_dst), D3DLOCK_DISCARD) < 0)
        return;
    ...
    {
        const auto L = 0.5f, R = io.DisplaySize.x+0.5f, T = 0.5f, B = io.DisplaySize.y+0.5f;
