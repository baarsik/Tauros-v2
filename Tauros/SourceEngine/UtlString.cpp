...
CUtlBinaryBlock::CUtlBinaryBlock(void* pMemory, int nSizeInBytes, int nInitialLength) : m_Memory(static_cast<unsigned char*>(pMemory), nSizeInBytes)
{
	m_nActualLength = nInitialLength;
}

CUtlBinaryBlock::CUtlBinaryBlock(const void* pMemory, int nSizeInBytes) : m_Memory(static_cast<const unsigned char*>(pMemory), nSizeInBytes)
{
...
void CUtlBinaryBlock::SetLength(int nLength)
{
	...
	if (nLength > m_Memory.NumAllocated()) {
		const auto nOverFlow = nLength - m_Memory.NumAllocated();
	...
	if (m_Memory.NumAllocated() > m_nActualLength) {
		memset(reinterpret_cast<char *>(m_Memory.Base()) + m_nActualLength, 0xEB, m_Memory.NumAllocated() - m_nActualLength);
	}
#endif
}
...
void CUtlBinaryBlock::Set(const void *pValue, int nLen)
{
	...
	if (m_nActualLength) {
		if (reinterpret_cast<const char *>(m_Memory.Base()) >= static_cast<const char *>(pValue) + nLen ||
			reinterpret_cast<const char *>(m_Memory.Base()) + m_nActualLength <= static_cast<const char *>(pValue)) {
...
void CUtlString::Set(const char *pValue)
{
	...
	const int nLen = pValue ? strlen(pValue) + 1 : 0;
...
int CUtlString::Format(const char *pFormat, ...)
{
	...
	auto len = _vsnprintf_s(tmpBuf, 4096, sizeof tmpBuf - 1, pFormat, marker);
	va_end(marker);
